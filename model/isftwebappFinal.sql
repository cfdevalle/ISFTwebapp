...
-- -----------------------------------------------------
-- Table `isftwebappfinal`.`equivalencias`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `isftwebappfinal`.`equivalencias` ;

CREATE  TABLE IF NOT EXISTS `isftwebappfinal`.`equivalencias` (
  `cod_materia_o` INT(3) NOT NULL ,
  `cod_carrera_o` INT(2) NOT NULL ,
  `cod_materia_d` INT(3) NOT NULL ,
  `cod_carrera_d` INT(2) NOT NULL ,
  PRIMARY KEY (`cod_materia_o`, `cod_carrera_o`, `cod_materia_d`, `cod_carrera_d`) ,
  INDEX `fk_table1_materia2_idx` (`cod_materia_d` ASC, `cod_carrera_d` ASC) ,
  CONSTRAINT `fk_table1_materia1`
    FOREIGN KEY (`cod_materia_o` , `cod_carrera_o` )
    REFERENCES `isftwebappfinal`.`materia` (`Cod_Materia` , `Cod_Carrera` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_table1_materia2`
    FOREIGN KEY (`cod_materia_d` , `cod_carrera_d` )
    REFERENCES `isftwebappfinal`.`materia` (`Cod_Materia` , `Cod_Carrera` )
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE  TABLE IF NOT EXISTS `isftwebappfinal`.`correlativa` (
  ...
  `Cod_Materia` INT(3) NOT NULL ,
  `Cod_Carrera` INT(2) NOT NULL ,
  PRIMARY KEY (`Cod_Materia`, `Cod_Carrera`, `Cod_Correlativa`) ,
